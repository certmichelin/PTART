...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if self.instance is None:
            ...
            if not self.instance.pentesters.exists() or ("pentesters" in self.initial_data and len(self.initial_data["pentesters"]) == 0):
                raise serializers.ValidationError(
                    {"pentesters": "At least one pentester is required"}
                )
        return data
...
class RetestScreenshotSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        caption = validated_data.pop("caption")
        return RetestScreenshot.objects.create(
            screenshot=screenshot,
            caption=caption,
...
class AttachmentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        return Attachment.objects.create(
            attachment=attachment, attachment_name=attachment_name, hit=hit
        )
...
class RetestHitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if "hit" not in self.initial_data:
            raise serializers.ValidationError({"hit": "Hit id is required."})

        try:
            campaign = RetestCampaign.objects.get(
                pk=self.initial_data["retest_campaign"]
            )
            hit = Hit.objects.get(pk=self.initial_data["hit"])
            if (
                campaign is None
                or hit is None
                or campaign.project.id != hit.assessment.project.id
            ):
                raise serializers.ValidationError(
                    {
            ...
            raise serializers.ValidationError({"hit": "Hit does not exist."})
        return data
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if "retest_campaign" in self.initial_data:
                retest_hit.retest_campaign = RetestCampaign.objects.get(
                    pk=self.initial_data["retest_campaign"]
                )
