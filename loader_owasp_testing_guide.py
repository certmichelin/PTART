...
if not current_methodology :
    ...
    methodology = Methodology(name=methodology_json['name'], description=methodology_json['description'])
    methodology.save()
    ...
    for category_key in owasp_json['categories'].keys():
        module_name = "{id} - {name}".format(id = owasp_json['categories'][category_key]["id"], name = category_key)
